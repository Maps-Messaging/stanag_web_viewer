...
export class StompTransport implements MessageTransport {
  ...
  private subscribeToStanag(): void {
    this.subscribe(this.configuration.droneTopic, (message) => this.handleStanagMessage(message));
    if (this.configuration.taskStatusTopic) {
      this.subscribe(this.configuration.taskStatusTopic, (message) => this.handleStanagMessage(message));
    }
  }

  private subscribeToTwins(): void {
    this.subscribe(TWIN_TOPIC, (message) => this.handleTwinMessage(message));
  }

  private subscribeToMavlinkStatus(): void {
    this.subscribe(MAVLINK_STATUS_TOPIC, (message) => this.handleMavlinkStatus(message));
  }
  ...
  private unsubscribeAll(): void {
    if (this.client?.connected) this.subscriptions.forEach((subscription) => subscription.unsubscribe());
    this.subscriptions = [];
  }
  ...
  private publishTaskPayload(task: DroneTask, payload: unknown): void {
    ...
    const destination = resolveTaskAdminDestination(this.configuration.taskAdminTopic, drone.name);
    this.publishJson(destination, payload);
  }
  ...
  private handleMavlinkStatus(message: IMessage): void {
    ...
    this.parse(message, (payload) => dispatchMavlinkStreamStatus(systemId, payload));
  }
